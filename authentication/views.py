...
def register(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(username=username).exists():
            ...
            context['has_error'] = True
            return render(request, "authentication/register.html", {'data':context['data']}, status=409)
        ...
        if context['has_error']:
            return render(request, "authentication/register.html", {'data':context['data']})
...
def login_user(request):
    if request.method == 'POST':
        ...
        if user and not user.is_email_verified:
            messages.add_message(request, messages.ERROR, "Email is not verified, check your inbox")
            return render(request, "authentication/login.html", {'data':context['data']}, status = 401)
...
def activate_user(request, uidb64, token):
    ...
    if user and generate_token.check_token(user, token):
        ...
        messages.add_message(request, messages.SUCCESS, f"Email verified, you can login")
        return redirect('authentication:login')
